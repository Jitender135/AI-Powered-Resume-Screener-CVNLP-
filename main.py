from fastapi import FastAPI, Request, UploadFile, File, Form
from fastapi.responses import HTMLResponse, StreamingResponse
from fastapi.templating import Jinja2Templates

import os
import shutil
from typing import List
import re
import zipfile
import io

# new: structured parser + hybrid matcher + explainer + planner
from app.NLP.ner_and_skills import parse_resume
from app.hybrid_matcher import hybrid_match
from app.explainers.explain import generate_explanation_payload
from app.planner import get_required_skills_for_job, evaluate_readiness, recommend_courses_for_skills

from app.resume_parser import extract_text_from_pdf
from app.preprocess import clean_resume_text
from app.gpt_matcher import get_resume_match_score
from app.cosine_matcher import cosine_match_score
# from app.key_matcher import keyword_match_score

import fitz
import docx2txt

LAST_RESULTS = []

app = FastAPI()
UPLOAD_DIR = "uploads"
os.makedirs(UPLOAD_DIR, exist_ok=True)
templates = Jinja2Templates(directory="app/templates")


def extract_years(text):
    match = re.search(r'(\d+)[ ]*[-to]{0,3}[ ]*(\d+)?[ ]*years?', text.lower())
    if match:
        start = int(match.group(1))
        end = int(match.group(2)) if match.group(2) else start
        return (start + end) // 2
    return None


@app.get("/", response_class=HTMLResponse)
async def form(request: Request):
    return templates.TemplateResponse("extract.html", {"request": request})


@app.get("/download-select", response_class=HTMLResponse)
async def download_select(request: Request):
    return templates.TemplateResponse("download_form.html", {"request": request})


@app.post("/download_zip")
async def download_cvs(request: Request, top_n: int = Form(...), zip_name: str = Form("")):
    zip_filename = zip_name.strip() or "top_resumes.zip"
    if not zip_filename.endswith(".zip"):
        zip_filename += ".zip"

    selected = LAST_RESULTS[:top_n]

    # Create in-memory zip
    zip_stream = io.BytesIO()
    with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zipf:
        for res in selected:
            filepath = os.path.join(UPLOAD_DIR, res['filename'])
            if os.path.exists(filepath):
                zipf.write(filepath, arcname=res['filename'])

    zip_stream.seek(0)

    return StreamingResponse(zip_stream, media_type="application/zip", headers={
        "Content-Disposition": f"attachment; filename={zip_filename}"
    })


@app.post("/extract", response_class=HTMLResponse)
async def extract(
    request: Request,
    files: List[UploadFile] = File(...),
    job_description: str = Form(""),
    jd_file: UploadFile = File(None),
    scoring_method: str = Form("gpt"),
    goal_title: str = Form("")   # <-- added goal_title input
):
    results = []

    jd_text = ""

    if jd_file is not None and jd_file.filename != "":
        jd_ext = jd_file.filename.lower().split('.')[-1]
        jd_path = os.path.join(UPLOAD_DIR, jd_file.filename)
        with open(jd_path, "wb") as f:
            shutil.copyfileobj(jd_file.file, f)

        if jd_ext == "pdf":
            with fitz.open(jd_path) as doc:
                jd_text = "".join([page.get_text() for page in doc])
        elif jd_ext == "docx":
            jd_text = docx2txt.process(jd_path)

    if not jd_text and job_description.strip():
        jd_text = job_description.strip()

    if not jd_text:
        return templates.TemplateResponse("extract.html", {
            "request": request,
            "error": "Please upload or paste a job description.",
            "goal_title": goal_title
        })

    cleaned_jd = clean_resume_text(jd_text)
    jd_exp = extract_years(cleaned_jd)

    for file in files:
        file_path = os.path.join(UPLOAD_DIR, file.filename)
        with open(file_path, "wb") as f:
            shutil.copyfileobj(file.file, f)

        resume_text = extract_text_from_pdf(file_path)
        cleaned_resume = clean_resume_text(resume_text)
        resume_exp = extract_years(cleaned_resume)

        if scoring_method == "gpt":
            match_raw = get_resume_match_score(cleaned_resume, cleaned_jd)
            match = {
                "score": match_raw.get("score", 0),
                "reason": match_raw.get("reason", ""),
                "breakdown": {},
                "missing_skills": [],
                "candidate": {}
            }

        elif scoring_method == "cosine":
            match_raw = cosine_match_score(cleaned_resume, cleaned_jd)
            match = {
                "score": match_raw.get("score", 0),
                "reason": match_raw.get("reason", ""),
                "breakdown": {
                    "semantic_score": match_raw.get("semantic_score", 0),
                    "experience_score": match_raw.get("experience_score", 0),
                    "keyword_score": match_raw.get("keyword_score", 0)
                },
                "missing_skills": [],
                "candidate": {}
            }

        elif scoring_method == "hybrid":
            # Use structured parsing + hybrid matcher (reasoning + minimal planning via missing_skills)
            candidate = parse_resume(cleaned_resume, filename=file.filename)
            match = hybrid_match(candidate, cleaned_jd)

        else:
            match = {
                "score": 0,
                "reason": "Invalid scoring method selected.",
                "breakdown": {},
                "missing_skills": [],
                "candidate": {}
            }

        # Ensure we have a candidate dict for planner (create if missing)
        candidate_obj = match.get('candidate') or parse_resume(cleaned_resume, filename=file.filename)

        # Planner: evaluate goal if provided
        plan_info = {}
        if goal_title and goal_title.strip():
            required = get_required_skills_for_job(goal_title)
            readiness = evaluate_readiness(candidate_obj.get('skills', []), required)
            course_suggestions = recommend_courses_for_skills(readiness['missing_skills'])
            plan_info = {
                "goal_title": goal_title,
                "required_skills": required,
                "matched_skills": readiness['matched_skills'],
                "missing_skills": readiness['missing_skills'],
                "readiness_pct": readiness['readiness_pct'],
                "course_suggestions": course_suggestions
            }

        # Generate explanation & recommendation payload (Step-5)
        payload = generate_explanation_payload(
            score=match.get('score', 0),
            breakdown=match.get('breakdown', {}),
            candidate=candidate_obj,
            missing_skills=match.get('missing_skills', [])
        )

        # Append richer result for template (sections will be used by the template)
        results.append({
            "filename": file.filename,
            "text": cleaned_resume,
            "score": match.get('score', 0),
            "reason": match.get('reason', ''),
            "sections": match.get('breakdown', {}),
            "missing_skills": match.get('missing_skills', []),
            "candidate": candidate_obj,
            "explanation_text": payload.get('explanation_text', ''),
            "recommendation": payload.get('recommendation', ''),
            "plan": plan_info
        })
        results.sort(key=lambda x: x['score'], reverse=True)
        LAST_RESULTS.clear()
        LAST_RESULTS.extend(results)

    return templates.TemplateResponse("extract.html", {
        "request": request,
        "results": results,
        "job_description": cleaned_jd,
        "scoring_method": scoring_method,
        "goal_title": goal_title
    })
